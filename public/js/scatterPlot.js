...
//     // set svg to scatterPlot
//     let scatterPlot = d3.select('#' + domElement).append('svg')
//         .attr('width', width)
//         .attr('height', height);
...
//      */
//     var colorScale = d3.scaleLinear().domain([0, 10]).range(['#383c4a', '#383c4a']); // Incase we want colour to change based on value


//     scatterPlotObj.loadAndRenderDataset = function (data) {
//         dataset = data;
//         xScale = d3.scaleLinear().domain([0, 10]);
//         yScale = d3.scaleLinear().domain([0, 11]);
//         draw();
//         return scatterPlotObj;
//     }
...
//         const binding = circles.selectAll('.data-point').data(dataset, d => d.id);
...
//                 .style("fill", "#4292c6");
//         }
//         const brushedNodes = dataset.filter(d =>
